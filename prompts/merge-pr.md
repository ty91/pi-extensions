...
1. Resolve the target PR from the argument above, recent conversation, or the current branch via `gh pr view`; ask only if still ambiguous.
2. Inspect the PR with `gh pr view` and stop if it cannot be found or is not mergeable.
3. Merge with `gh pr merge`, preferring the repository's normal non-interactive merge method and deleting the remote branch when possible.
4. Verify the PR is merged with `gh pr view`.
